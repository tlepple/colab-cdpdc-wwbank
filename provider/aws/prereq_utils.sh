...
terminate_all_ec2() {
  ...
  terminate_id=`aws --output json --region ${AWS_REGION:?} ec2 describe-instances --filters "Name=tag:Name,Values=horizon-cdpdc-wwbank-${OWNER_TAG:?}" | jq -r ".Reservations[].Instances[].InstanceId"`
...
create_onenode_instance() {
	...
	aws --region ${AWS_REGION:?} ec2 create-tags --resources ${oneNodeInstanceId:?} --tags Key=owner,Value=${OWNER_TAG:?} Key=Name,Value=horizon-cdpdc-wwbank-${OWNER_TAG:?} Key=enddate,Value=${ENDATE_TAG:?} Key=project,Value='personal development'
